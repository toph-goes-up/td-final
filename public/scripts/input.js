//IIFE to add input functionality to the game
App.input = (function() {
    //Function Expression for mouse controls
    function Mouse(){
        let that = {
            mouseDown: [],
            mouseUp: [],
        ...
        //Empty out the event arrays and run their handlers
        ...
        //Register event listeners to capture mouse input
    ...
    //Function expression for keyboard controls
    function Keyboard(){
        let that = {
            keys: {},
            handlers: []
        ...
        //Register commands using a keycode number and handler function
        that.registerCommand = function(key, singleShot, handler){
            that.handlers.push({key: key, singleShot: singleShot, handler: handler});
        };

        //Empty out all queued commands and run their handlers
        that.processInput = function(elapsedTime){
            for(let i = 0; i < that.handlers.length; i++){
                ...
                if(that.keys.hasOwnProperty(handler['key'])){
                    ...
                    if(handler.singleShot){
                        delete that.keys[handler['key']]
                    }
                }
            }
        };
...
// Helper object (dictionary) for setting keyboard commands
